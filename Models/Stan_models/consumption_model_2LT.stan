...
functions{
  ...
  // Function for saptial autocorrelation (Gaussian process)
...
///// Prediction
  ...
  matrix[N_sites_pred,N_scenarios] ED_pred;                         // factor - for each scenario, proportion of people with education > primary AND  proportion of people with education < secondary, in each cell
  matrix[N_location_types,N_scenarios] LT_pred [N_sites_pred];      // factor - for each scenario, proportion of people living in rural or urban (towns and cities) in each cell
...
parameters {
  ...
  real<lower=0> sigma;                           // uncertainty if an entire year was surveyed
  vector [N_households] tau;                     // non-centered paramterization 
  
  real<lower=0> theta;                           // overdispersion parameter, gamma distribution
  
  real<lower=0> zeta;                            // gaussian process period 1
  real<lower=0> rho;                             // gaussian process period 1
  vector [N_sites] eta;                          // gaussian process period 1
   
  real<lower=0> xi;                              // gaussian process period 2
  real<lower=0> omicron;                         // gaussian process period 2
  vector [N_sites] omega;                        // gaussian process period 2
  
  vector<lower=0> [N_missing_AME] AME_imputed;   // AME imputation - vector of imputed values
  real nu;                                       // AME imputation - mean of imputed values
  real<lower=0> psi;                             // AME imputation - standard deviation of imputed values
...
generated quantities{
  ...
  for (j in 1:N_sites_pred){
    ...
    pred_a1[j,1] = a1[1] * LT_pred[j,1,1] + a1[2] * LT_pred[j,2,1];
    pred_a1[j,2] = a1[1] * LT_pred[j,1,2] + a1[2] * LT_pred[j,2,2];
    pred_a1[j,3] = a1[1] * LT_pred[j,1,3] + a1[2] * LT_pred[j,2,3];
    
    pred_b1[j,1] = b1[1] * LT_pred[j,1,1] + b1[2] * LT_pred[j,2,1];
    pred_b1[j,2] = b1[1] * LT_pred[j,1,2] + b1[2] * LT_pred[j,2,2];
    pred_b1[j,3] = b1[1] * LT_pred[j,1,3] + b1[2] * LT_pred[j,2,3];
    
    pred_c3[j,1] = c3[1] * LT_pred[j,1,1] + c3[2] * LT_pred[j,2,1];
    pred_c3[j,2] = c3[1] * LT_pred[j,1,2] + c3[2] * LT_pred[j,2,2];
    pred_c3[j,3] = c3[1] * LT_pred[j,1,3] + c3[2] * LT_pred[j,2,3];
    
    pred_a5[j,1] = a5[1] * (1 - ED_pred[j,1]) + a5[2] * ED_pred[j,1];
    pred_a5[j,2] = a5[1] * (1 - ED_pred[j,2]) + a5[2] * ED_pred[j,2];
    pred_a5[j,3] = a5[1] * (1 - ED_pred[j,3]) + a5[2] * ED_pred[j,3];
    
    pred_b5[j,1] = b5[1] * (1 - ED_pred[j,1]) + b5[2] * ED_pred[j,1];
    pred_b5[j,2] = b5[1] * (1 - ED_pred[j,2]) + b5[2] * ED_pred[j,2];
    pred_b5[j,3] = b5[1] * (1 - ED_pred[j,3]) + b5[2] * ED_pred[j,3];
    
    pred_c2[j,1] = c2[1] * (1 - ED_pred[j,1]) + c2[2] * ED_pred[j,1];
    pred_c2[j,2] = c2[1] * (1 - ED_pred[j,2]) + c2[2] * ED_pred[j,2];
    pred_c2[j,3] = c2[1] * (1 - ED_pred[j,3]) + c2[2] * ED_pred[j,3];
